...
public interface RoleDao extends BaseMapper<RolePO> {
    ...
    @Select("SELECT * FROM mate_role WHERE role_key = #{roleKey} AND deleted = 0")
    RolePO selectByRoleKey(@Param("roleKey") String roleKey);

    @Select("SELECT COUNT(1) > 0 FROM mate_role WHERE role_key = #{roleKey} AND deleted = 0")
